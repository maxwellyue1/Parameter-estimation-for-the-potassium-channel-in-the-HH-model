...
def add_dim(inputs, n_traces, n_points, dim = 2): 
    '''
    adding a dim to dataset, default = 2, so theres one channel for time, one for current
    '''
    return torch.reshape(inputs, (inputs.shape[0], 2, n_traces, n_points))

# initialize train, val, test set
X_train = add_dim(dataset[dataset.train_set.indices][0], 12, 20)
Y_train = dataset[dataset.train_set.indices][1]

X_val = add_dim(dataset[dataset.val_set.indices][0], 12, 20)
Y_val = dataset[dataset.val_set.indices][1]

X_test = add_dim(dataset[dataset.test_set.indices][0], 12, 20)
Y_test = dataset[dataset.test_set.indices][1]

# initialize NN model
model = LeNet5(dataset.params.shape[1]).to(device)

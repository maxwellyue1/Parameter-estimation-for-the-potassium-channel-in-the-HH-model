...
def rnn_add_dim(inputs, n_points=20, n_traces=12): 
    '''
    adding a dim to dataset, default = 2, so theres one channel for time, one for current
    '''
    return inputs.view(-1, n_traces*2, n_points).permute(0, 2, 1).contiguous()

# initialize train, val, test set
X_train = rnn_add_dim(dataset[dataset.train_set.indices][0])
Y_train = dataset[dataset.train_set.indices][1]

X_val = rnn_add_dim(dataset[dataset.val_set.indices][0])
Y_val = dataset[dataset.val_set.indices][1]

X_test = rnn_add_dim(dataset[dataset.test_set.indices][0])
Y_test = dataset[dataset.test_set.indices][1]

# set up model hyper-parameters
input_size = 24 # numer of traces times 2
hidden_size = 128
n_layers = 1
mlp_hidden_size = 64
n_classes = 8 # 8 parameters to estimate

# initialize NN model
model = RNN(input_size, hidden_size, n_layers, mlp_hidden_size, n_classes).to(device)
...
norm_diff_weights_2d_tensor = torch.norm(diff_weights_2d_tensor, dim=1).detach().numpy()
